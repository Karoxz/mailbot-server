...
# email reply on that thread. This is the foundation for rate/lane/
# broker analytics and the future bidding-recommendation engine.
#
# Pattern deliberately mirrors license_db.py: module-level DB_PATH next
# to this file, plain sqlite3.connect() per call (no ORM, no pooling —
# SQLite handles that fine at this volume), init_db() called once from
# the FastAPI lifespan.
#
# Outcome model:
#   status starts 'pending' the moment a bid is sent (BID PC/PHONE/DRAFT
#   click). A later step (broker-reply watcher, not in this module) will
#   call update_bid_outcome() once a reply on the same Gmail thread is
#   classified — 'won' / 'lost' / 'no_response' / 'countered'.
# =============================================================
...
def record_bid(order_id: str,
                ...
                verified_miles: float = None,
                verified_source: str = None,
                bid_amount: float = None) -> int:
    """
...
def get_pending_bids_for_thread(thread_id: str) -> list:
    """
    Used by the (future) broker-reply watcher: given a Gmail thread_id
    that just got a new message, find any bids on that thread still
...
def expire_stale_pending(older_than_days: int = 3) -> int:
    ...
    not that it's still live. Call this periodically (e.g. from the
    same 30s cache-flush pattern used elsewhere) so 'pending' stays a
    meaningful signal rather than an ever-growing junk drawer.
    """
